...
type NormalizedDeadlineFilters = Pick<
   DeadlineFiltersInput,
   'processId' | 'cnjNumber' | 'witnessId' | 'type' | 'status'
> & {
   dueDateFrom?: Date;
   dueDateTo?: Date;
   page: number;
...
export class DeadlinesRepository {
   async findMany(
      ...
      // If filtering by CNJ, resolve processId first
      let resolvedProcessId = filters.processId;
      if (filters.cnjNumber && !resolvedProcessId) {
         const [match] = await db
            .select({ id: processes.id })
            .from(processes)
            .where(ilike(processes.cnjNumber, `%${filters.cnjNumber}%`))
            .limit(1);
         if (!match)
            return {
               items: [],
         ...
         resolvedProcessId = match.id;
      }

      const conditions = [
         resolvedProcessId
            ? eq(deadlines.processId, resolvedProcessId)
            : undefined,
         filters.witnessId
      ...
      const [deadline] = await executor
         ...
         .set({
            ...
            dueDate: input.dueDate
               ? toDateColumnValue(input.dueDate)
               : undefined,
            updatedAt: new Date(),
         })
         .where(eq(deadlines.id, id))
   ...
   async cancelActiveByWitnessId(
      ...
      const cancelledDeadlines = await executor
         ...
         .where(
            and(
               eq(deadlines.witnessId, witnessId),
               eq(deadlines.status, 'aberto'),
            ),
         )
